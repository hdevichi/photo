...
public class FilePhotoFactory extends PhotoFactory {
	...
	public PhotoMetadata getExifMetaData( String id ) throws PhotoException {
		...
		try {
			...
			String focal = exifDirectory.getString(ExifDirectory.TAG_FOCAL_LENGTH);
			String dateTime = exifDirectory.getString(ExifDirectory.TAG_DATETIME);
			String exposure = exifDirectory.getString(ExifDirectory.TAG_EXPOSURE_TIME);
			String iso = exifDirectory.getString(ExifDirectory.TAG_ISO_EQUIVALENT);
			...
			try {
				Float aperture = exifDirectory.getFloat(ExifDirectory.TAG_APERTURE);
				float roundedAperture = Math.round(aperture*100);
				meta.aperture = "1/" + (roundedAperture/100);
			} catch (Exception e) {
				meta.aperture = exifDirectory.getString(ExifDirectory.TAG_APERTURE);
			}
			return meta;
