...
<%@page import="org.devichi.photo.control.servlet.AdminDirControl"%>
<%@page import="org.devichi.photo.control.servlet.AjaxServlet"%>
<HTML>
				...
				<table class="form" >
					...
					<tbody id="uploadLines">
						<tr>
							...
							<td align="left" >
								<input id="firstUpload" type="file" onchange="addUploadLine(1);" size="50" name="<%=AdminDirControl.FIELD_IMAGE %>1" <%if (photoConfig.getPhotoFactory().getFreeSpace() < org.devichi.photo.model.config.PhotoConfigurationConstants.MINIMUM_DISK_SPACE) {%> disabled <%}%>>
					...
					<tr>
						<td colspan="2" align="right">
						 	<%if (photoConfig.getPhotoFactory().getFreeSpace() < org.devichi.photo.model.config.PhotoConfigurationConstants.MINIMUM_DISK_SPACE) {%> 
						 	<fmt:message key="adminDir.addPicImpossible"/>
						 	<%} %>
							<input id="uploadButton" type="button" onclick="doUpload();" name="uploadButton" value="<fmt:message key="adminDir.submitAddImage"/>" <%if (photoConfig.getPhotoFactory().getFreeSpace() < org.devichi.photo.model.config.PhotoConfigurationConstants.MINIMUM_DISK_SPACE) {%> disabled <%}%>>
				...
				<table class="form" >
					...
					<tr><td>&nbsp;</td><td colspan="2">
						<div >
								...
								org.devichi.photo.utils.PhotoUtils.generateFolderTreeMarkup("admin_tree","selectFolderDirMove",photoConfig.getPhotoFactory().getRoot(), buffer, user.getLanguage());%>
								<%=buffer.toString() %>
						</div>
					</td></tr>
			...
			<tr><td colspan="2" width="100%">
				<table class="form"  width="100%">
					...
					<td colspan="3">
						<div >
								<%StringBuffer buffer = new StringBuffer();
								org.devichi.photo.utils.PhotoUtils.generateFolderTreeMarkup("admin_tree2", "selectFolderDirChildrenMove",photoConfig.getPhotoFactory().getRoot(), buffer, user.getLanguage());%>
								<%=buffer.toString() %>
						</div>
					</td></tr>
					<tr>
